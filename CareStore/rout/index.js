...
router.post("/register", body('email').isEmail().withMessage('ایمیل نامعتبر است'), body('password').matches("^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9]).{6,16}$").withMessage('پسورد باید شامل یک حرف بزرگ و عدد باشد'), async (req, res) => {
...
router.post("/login", async (req, res) => {
    ...
    if (!found_email1) {
        ...
        if (!find) {
            ...
            const token = jwt.sign({ email: found_email1.email, id: found_email1._id }, process.env.secret, {
                algorithm: "HS256",
            })
...
router.post("/product/add_commnet", async (req, res) => {
    name = req.body.name;
    email = req.body.email;
...
router.post("/product/show_comment", async (req, res) => {
    pro_id = req.body.pro_id;
...
router.post("/product/show_comment2", async (req, res) => {
...
router.post("/dash2", async (req, res) => {
    token = req.body.token;
    const user = jwt.verify(token, process.env.secret);
