...
export default {
  ...
  methods: {
    ...
    SendComment() {
      ...
      if (this.code != this.rand) {
        ...
        axios
          .post("http://localhost:9856/product/add_commnet", {
            name: this.name,
            email: this.email,
  ...
  mounted() {
    ...
    this.newlink = this.link.substring(27, 51);
    axios
      .post("http://localhost:9856/product/", {
        link: this.newlink,
      })
      .then((response) => {
        const status = JSON.parse(response.status);
    ...
    axios
      .post("http://localhost:9856/product/show_comment", {
        pro_id: this.newlink,
      })
      .then((response) => {
        const status = JSON.parse(response.status);
